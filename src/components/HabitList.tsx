...
export function HabitList({ habits, deleteHabit, toggleHabit }: HabitListProps) {
    ...
    return (
        <div className="flex flex-col gap-3">
            {habits.map(habit => (
                <HabitItem deleteHabit={deleteHabit} toggleHabit={toggleHabit} key={habit.id} habit={habit}></HabitItem>
            ))}
        </div>
    ) 
}
        ...
        <div className="rounded-xl bg-zinc-800 p-4 flex flex-col gap-3">
            ...
            <div className="flex gap-1.5">
                {visibleDates.map(date => (
                    <Button 
                        className="flex flex-1 flex-col items-center gap-0.5 rounded-1g text-xs"
